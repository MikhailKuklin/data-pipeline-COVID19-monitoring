...
credentials_block.save("covid-gcp-creds", overwrite=True)


bucket_block = GcsBucket(
    gcp_credentials=GcpCredentials.load("covid-gcp-creds"),
    bucket="dtc_data_lake_prime-framing-374716",  # insert your  GCS bucket name
)

bucket_block.save("covid-gcs", overwrite=True)
